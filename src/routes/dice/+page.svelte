...
<h1 class="mt-4 font-extrabold leading-tight text-center text-white text-7xl sm:text-8xl">Dice</h1>
	...
	<div class="px-4 py-8 sm:px-10">
		<div class="mt-6">
			<div class="w-full space-y-6">
				<div class="w-full">
					<select
						bind:value={selectedDice}
						on:change={() => {
							diceSite = dice(selectedDice);
							selected = true;
						}}
					...
					>
						<option selected disabled> Select an dice </option>
					...
					{#if selected}
						<div>
							<span
								class="block w-full rounded-md shadow-sm py-2 px-4 bg-violet-600 text-white w-full transition ease-in duration-200 text-center text-base font-semibold shadow-md focus:outline-none focus:ring-2 focus:ring-offset-2 rounded-lg"
								id="dice-site"
							>
								{#if diceSite != 0}
									{diceSite}
